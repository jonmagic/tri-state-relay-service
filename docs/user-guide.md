...
`relay voice ensure --line "Agent One"` performs the same sticky assignment and prints the selected voice id without enqueueing or speaking a relay. It prints nothing and exits successfully when the active provider does not support automatic line voices.

`relay voice ensure --line "Agent One" --voice-id "af_aoede"` explicitly assigns a voice from the active provider catalog. Relay validates the voice ID and persists the generic line-to-voice mapping without enqueueing or speaking a relay; applications remain responsible for their own line names and voice choices.
